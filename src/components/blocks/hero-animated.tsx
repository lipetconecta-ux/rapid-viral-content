...
const heroVariants = cva("relative min-h-svh w-full overflow-hidden", {
  variants: {
    layout: {
      default:
        "flex flex-col items-center text-center justify-center place-content-center",
...
export function BgGradient({
  ...
  const colorsArr = Array.isArray(gradientColors)
    ? gradientColors
    : GRADIENT_COLORS[gradientColors];
  const gradientString = colorsArr
  ...
  const dominantColor = colorsArr[colorsArr.length - 1].color;

  return (
    <div
      aria-hidden
      className={cn("absolute inset-0 -z-10", className)}
      style={{ background: gradientStyle, backgroundColor: dominantColor }}
      {...props}
    />
  );
}
...
function Word({ word, transition = TRANSITION_CONFIG, direction = "bottom" }: WordProps) {
  ...
  const variants = transformVariants(direction);
  return (
...
export function TextStagger({
  ...
  const MotionComp = motion.create(Component as React.ElementType);
